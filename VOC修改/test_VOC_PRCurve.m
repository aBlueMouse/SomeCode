...
%     cls = '1';
	...
	% [gtids,~]=textscan(VOCopts.imgsetpath,'%s%s');%gtids为测试集中图像的编号向量
	...
	%     a = [VOCopts.annopath num2str(gtids{i}) '.xml']
		...
		npos=npos+sum(~gt(i).diff);%记录整个测试集中非困难的目标总数（完整的目标？）总个数
	end
	% % % % % % % % % % % % % % % % % % % % % % 
	...
	for jpg_id=1:length(det_num)%一幅一幅将检测图像结果进行处理
        ...
        zj = [];
        for leibie = 1 : length(index)
            if char(class_id(leibie)) == cls
                zj = [zj (index(1)-1+leibie)];
            end
        end        
...
% 			end
			...
			for j=1:size(gt(gt_index).BB,2)%对所有groundTruth逐一比较
				bbgt=gt(gt_index).BB(:,j);%gt中第j个gt目标
    ...
    recall = sum(tp_all)/npos;
    precision = sum(tp_all)/(sum(tp_all)+sum(fp_all));
    
    recall_z(lei) = recall;
    precision_z(lei) = precision;
...
disp(recall_z);
disp(precision_z);
disp(['检出率:', num2str(mean(recall_z)*100), '%']);
disp(['准确率:', num2str(mean(precision_z)*100), '%']);
