...
from network_multiscale import Network

from dataset_multiscale import Preprocessing
...
def test_net(tester, logger, dets, det_range):
    ...
    while img_start < det_range[1]:
        ...
        for test_id in range(0, len(test_data), batch_size):
            ...
            # test_imgs = []
            details = []
            res_avg = np.zeros((end_id-start_id, cfg.output_shape[0], cfg.output_shape[1], cfg.nr_skeleton), dtype=np.float32)
            multiplier = [1.0, 1.2, 1.4]
            for m in range(len(multiplier)):
                scale = multiplier[m]
                test_imgs = []
                for i in range(start_id, end_id):
                    test_img, detail = Preprocessing(test_data[i], stage='test', scale=scale)
                ...
                for i in range(end_id - start_id):
                    mmp = res[i].transpose((1, 2, 0))
                    mmp = cv2.resize(mmp, (cfg.output_shape[1], cfg.output_shape[0]))
                    # print res_avg[i].shape
                    # print mmp.shape
                    res_avg[i] += mmp / len(multiplier)

            res = res_avg.transpose(0, 3, 1, 2)
            details = details[:(end_id-start_id)]
            details = np.asarray(details)
...
def test(test_model, logger):
    ...
    test_ann_path = 'dress_with_bbox.csv'
    ...
    for i in range(len(anns)):
        ...
        results.append(row)
    write_csv('result_dress_0529.csv', results)
