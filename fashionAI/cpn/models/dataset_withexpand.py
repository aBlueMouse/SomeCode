...
def Preprocessing(d, stage='train'):
    ...
    if stage == 'test':
        ...
        min_x = center_x - (bbox_w * expand / 2)
        max_x = center_x + (bbox_w * expand / 2)
        min_y = center_y - (bbox_h * expand / 2)
        max_y = center_y + (bbox_h * expand / 2)
        min_x = max(0, int(min_x))
        max_x = min(ori_width - 1, int(max_x))
        min_y = max(0, int(min_y))
        max_y = min(ori_height - 1, int(max_y))
